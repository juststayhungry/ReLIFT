...
class RewardManager():
    ...
    def __call__(self, data: DataProto):
        ...
        def process_item(args):
            ...
            valid_prompt_ids = prompt_ids[-valid_prompt_length:]#prompt_ids[seq_len - valid_prompt_length:]
            # 通过 attention_mask 计算实际有效的提示词长度（排除填充 token）
            response_ids = data_item.batch['responses'] 
            valid_response_length = data_item.batch['attention_mask'][prompt_length:].sum()
            valid_response_ids = response_ids[:valid_response_length]#有效提示词的id（去除后面的padding）
            # decode
...
def main(config):
    ...
    # 假设 config 是 omegaconf.DictConfig 或 Structured Config
    if hasattr(config, 'actor_rollout_ref') and dual_actor:
...
def main_task(config):
    ...
    from verl.trainer.ppo.ray_trainer import ResourcePoolManager, Role#todo
    ...
    elif config.dual_actor:
        #tlx
        role_worker_mapping = {
            Role.ActorRollout: ray.remote(ReLIFTActorRolloutRefWorker),
            Role.ActorRollout2: ray.remote(ReLIFTActorRolloutRefWorker),# 复用同一个类
            Role.Critic: ray.remote(CriticWorker),
        }
        mapping = {
            Role.ActorRollout: global_pool_id,
            Role.ActorRollout2: global_pool_id,
            Role.Critic: global_pool_id,
        }
    else:
        role_worker_mapping = {
