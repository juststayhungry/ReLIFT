...
from verl.utils.dataset.my_sft_dataset import MySFTDataset
...
class MyFSDPSFTTrainer(object):
    ...
    def _build_dataloader(self):
        config = self.config
        # build dataset
        self.train_dataset = MySFTDataset(parquet_files=config.data.train_files,
        ...
        self.val_dataset = MySFTDataset(parquet_files=config.data.val_files,
                                      tokenizer=self.tokenizer,
                                      prompt_key=config.data.prompt_key,
                                      prompt_dict_keys=config.data.get('prompt_dict_keys', None),
                                      response_key=config.data.response_key,
...
def main(config):
    ...
    trainer = MyFSDPSFTTrainer(config=config, device_mesh=device_mesh)
    trainer.fit()
