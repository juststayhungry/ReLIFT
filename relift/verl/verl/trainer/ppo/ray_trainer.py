...
class RayPPOTrainer(object):
    ...
    def init_workers(self):
        ...
        #dual actor 
        if self.dual_actor:
            resource_pool = self.resource_pool_manager.get_resource_pool(Role.ActorRollout2)
            actor_rollout_cls2 = RayClassWithInitArgs(cls=self.role_worker_mapping[Role.ActorRollout2],
        ...
        if self.dual_actor:
            self.actor_rollout_wg2 = all_wg['actor_rollout2']
            self.actor_rollout_wg2.init_model()
    ...
    def _save_checkpoint(self):
        ...
        if self.dual_actor:
            actor_local_path2 = os.path.join(local_global_step_folder, 'actor2')
            actor_remote_path2 = None if self.config.trainer.default_hdfs_dir is None else os.path.join(
                self.config.trainer.default_hdfs_dir, f'global_step_{self.global_steps}', 'actor2')
            self.actor_rollout_wg2.save_checkpoint(actor_local_path2, actor_remote_path2, self.global_steps)
